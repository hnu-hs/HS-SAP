...
class MySQL:
	...
	def insertmany(self,sql,sqlist):
		u'执行 INSERT 语句。如主键为自增长int，则返回新生成的ID'
		try:
			self._cur.execute("SET NAMES utf8")
			self._cur.executemany(sql,sqlist)
			self._conn.commit()
			return self._conn.insert_id()
		except MySQLdb.Error, e:
			self.error_code = e.args[0]
	...
	def call_procedure_sp(self,spname,params):
         u'调用存储过程'
         try:
             
             self._cur.execute("SET NAMES utf8")
             
             #params = (10,0,0,'|880201,黑龙江,|880202,新疆板块,|880203,吉林板块,|')
             
             self._cur.callproc('sp_boardIndex_bulid',params)
             ...
             #self._cur.execute('select @_proctest_2,@_proctest_3')
             
             #self._conn.autocommit(1)
             
             #self._cur.execute('call sp_boardIndex_bulid(%s,%s,%s,%s)',(10,0,0,'|880201,黑龙江,|880202,新疆板块,|880203,吉林板块,|'))
