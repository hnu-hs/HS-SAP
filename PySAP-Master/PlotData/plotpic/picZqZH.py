...
class picExcel():
    ...
    def picModel(self,wbkFlag=0):
        ...
        for i in xrange(len(indexZf)): 
            try:
                sheet.write_row(top+i+2,left,[indexZf.iat[i,0],indexZf.iat[i,1],indexZf.iat[i,2]],PER)                                  
            except:
                print indexZf.iat[i,0]    
                
        for i in xrange(len(indexDf)): 
            try:
                sheet.write_row(top+i+2,left+4,[indexDf.iat[i,0],indexDf.iat[i,1],indexDf.iat[i,2]],PER)                                  
            except:
        ...
        for i in xrange(len(indexTldx)): 
            try:
                sheet.write_row(top+i+2,left+8,[indexTldx.iat[i,2],indexTldx.iat[i,0]],TLDX)                                  
            except:
                print indexTldx.iat[i,2]        
                
        for i in xrange(len(indexTldxTop5)): 
            try:
                sheet.write_row(top+i+2,left+11,[indexTldxTop5.iat[i,2],indexTldxTop5.iat[i,0]],TLDX)                                  
            except:
                print indexTldxTop5.iat[i,2]                     

        for i in xrange(len(indexTldxTail5)): 
            try:
                sheet.write_row(top+i+2,left+14,[indexTldxTail5.iat[i,2],indexTldxTail5.iat[i,0]],TLDX)                                  
            except:
                print indexTldxTail5.iat[i,2] 
                
                
        #写板块Q因子
        for i in xrange(len(indexQ)): 
            try:
                sheet.write_row(top+i+2,left+17,[indexQ.iat[i,2],indexQ.iat[i,0]],ZW)                                  
            except:
                print indexTldx.iat[i,0]        
                
        for i in xrange(len(indexQTop5)): 
            try:
                sheet.write_row(top+i+2,left+20,[indexQTop5.iat[i,2],indexQTop5.iat[i,0]],ZW)                                  
            except:
                print indexTldxTop5.iat[i,0]                     

        for i in xrange(len(indexQTail5)): 
            try:
                sheet.write_row(top+i+2,left+23,[indexQTail5.iat[i,2],indexQTail5.iat[i,0]],ZW)                                  
            except:
       ...
       # QTail=self.sQ.tail(10)[::-1]
        ...
        for board in boardNames:
            ...
            for i in xrange(dataLen):
                try:
                    sheet2.write(top+i,left+1,data.iat[i,4],paiming)
                    sheet2.write_row(top+i,left+2,[data.iat[i,0],data.iat[i,1],data.iat[i,2]],PER)                                  
                except:
                    print data.iat[i,0]                                                                                                  
            top+=dataLen+1
        ...
        for board,data in stock200ChgGrouped:
           # xdData=stock200XdChg[stock200XdChg.board_name==board]
            ...
            for i in xrange(dataLen):
                try:
                    sheet2.write(top+i,left+6,data.iat[i,4],paiming)
                    sheet2.write_row(top+i,left+7,[data.iat[i,0],data.iat[i,1],data.iat[i,2]],PER)                                                         
                except:
                    print data.iat[i,1]
        ...
        for board in boardNames:
            ...
            for i in xrange(dataLen):
                try:
                    sheet2.write_row(top+i,left+30,[data.iat[i,2],data.iat[i,0]],ZW)                                  
                except:
                    print data.iat[i,0]                                                                                                  
            top+=dataLen+1                
        ...
        for board,data in stock200QGrouped:
           # xdData=stock200XdChg[stock200XdChg.board_name==board]
            ...
            for i in xrange(dataLen):
                try:
                    sheet2.write_row(top+i,left+33,[data.iat[i,2],data.iat[i,0]],ZW)                                                         
                except:
                    print data.iat[i,0]
