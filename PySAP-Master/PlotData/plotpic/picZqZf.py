...
class picExcel():
    ...
    def __init__(self,period,indexChg,stockChg,stock200Chg):
    #stockChg,stock200Chg):
        self.filename=u'E:/test/报表/周期涨幅'+period+'.xlsx'
    ...
    def picModel(self):
        ...
        sheet.merge_range(0,left,0,left+17,self.period,red)
                      
        #写每天的标题头    
        sheet.merge_range(top,left,top,left+2,'行业涨幅排序',bigwhite1)
        ...
        sheet.merge_range(top,left+4,top,left+16,'股票涨幅排序',bigwhite2)            
        ...
        indexChg=self.indexChg
        #indexXdChg=self.indexChg[1]
        
        stockChg=self.stockChg
        stockTop20=stockChg.head(300)
        #stockXdChg=self.stockChg[1]
        
        stock200Chg=self.stock200Chg
        #stock200XdChg=self.stock200Chg[1]
                      
        #写板块排名
        for i in xrange(len(indexChg)): 
            try:
                sheet.write(top+i+2,left,indexChg.iat[i,0],ZW)
                sheet.write(top+i+2,left+1,indexChg.iat[i,1],ZW)
                #sheet.write(top+i+2,left+3,indexXdChg.iat[i,0],ZW)                
                sheet.write(top+i+2,left+2,indexChg.iat[i,2],ZW)                                   
            except:
        ...
        for board in boardNames:
            ...
            for i in xrange(dataLen):
                try:
                    sheet.write(top+i,left+5,data.iat[i,4],paiming)
                    sheet.write(top+i,left+6,data.iat[i,0],ZW)
                    sheet.write(top+i,left+7,data.iat[i,1],ZW)
                    #sheet.write(top+i,left+10,xdData.iat[i,0],ZW)                
                    sheet.write(top+i,left+8,data.iat[i,2],ZW)                                   
                except:
                    print data.iat[i,1]
...
#                    sheet.write(top+i,left+15,data200.iat[i,1],ZW)
#                    sheet.write(top+i,left+17,xdData200.iat[i,0],ZW)                
#                    sheet.write(top+i,left+18,xdData200.iat[i,1],ZW)                                   
#                except:
#                    print board
#                                                           
#            top+=(dataLen200+1)      
        ...
        for board,data in stock200ChgGrouped:
            ...
            for i in xrange(dataLen):
                try:
                    sheet.write(top+i,left+10,data.iat[i,4],paiming)
                    sheet.write(top+i,left+11,data.iat[i,0],ZW)
                    sheet.write(top+i,left+12,data.iat[i,1],ZW)
                    #sheet.write(top+i,left+17,xdData.iat[i,0],ZW)                
                    sheet.write(top+i,left+13,data.iat[i,2],ZW)                                   
                except:
                    print board
